...
function renderPcCards() {
    ...
    card.innerHTML = `
      ...
      <div class="pc-actions">
        ...
        <div class="topup-group">
          ...
          <input type="number" min="0" step="50" value="${topupDraft}" data-index="${index}" class="topup-input" />
          <button data-index="${index}" class="topup-btn">Пополнение</button>
        </div>
  ...
  document.querySelectorAll(".topup-btn").forEach((button) => {
    button.addEventListener("click", () => {
      const index = Number(button.dataset.index);
      const pc = pcsState[index];
      const input = document.querySelector(`.topup-input[data-index="${index}"]`);
      draftTopups[pc.id] = input.value;
      const amount = Number(String(input.value || "0").replace(",", "."));
      if (amount > 0) {
        pcsState[index].balance += amount;
        pcsState[index].lastEvent = `Баланс пополнен на ${amount} ₽`;
        draftTopups[pc.id] = "0";
        savePcs(pcsState);
        renderPcCards();
      } else {
        pcsState[index].lastEvent = "Введите сумму пополнения больше 0";
        savePcs(pcsState);
        renderPcCards();
      }
    });
  ...
  document.querySelectorAll(".topup-input").forEach((input) => {
    input.addEventListener("input", () => {
      const index = Number(input.dataset.index);
      draftTopups[pcsState[index].id] = input.value;
    });
  });
